...
def generate_fallback_summary(audit_data):
    ...
    # Guess type
    is_shop = "Shopify" in audit_data['stack'] or "WooCommerce" in audit_data['stack'] or "BigCommerce" in audit_data['stack']
    
    if is_shop:
        summary = f"""
### 1. Executive Summary
This **E-commerce** site using {audit_data['stack']} is technically accessible but lacks key Agentic protocols. The absence of an **ai.txt** file means AI buyers have no clear rules for engagement. Without specific permissions, automated **transactions** and product discovery may be unreliable.
...
"""
        ...
        summary = f"""
### 1. Executive Summary
This site runs on {audit_data['stack']} and lacks essential **Agentic** standards. The absence of an **ai.txt** file prevents controlled **content retrieval** by AI systems. This limits the site's ability to be accurately cited by LLMs for **lead generation** and answers.
...
* **Risk:** Reduced organic traffic from AI-powered search engines like SearchGPT and Gemini.
"""
...
# FIX FOR STICKY INPUT: We use 'key' to sync state, but NO 'value' to force it back.
url_input = st.text_input("Enter Client Website URL", placeholder="https://www.example.com", key="url_input_widget")
...
if st.button("🚀 Run Full Audit"):
    if not api_key:
        ...
        # Clear previous data to force fresh view
        st.session_state['audit_data'] = None
        st.session_state['recs'] = None
        st.session_state['ai_summary'] = None
        
        # Run Audit
        data, r, s = perform_audit(url_input, api_key)
...
if st.session_state['audit_data']:
    ...
    with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
        df = pd.DataFrame({
            "Metric": ["URL", "Stack", "Robots.txt", "AI.txt", "Schema"],
            "Status": [
                ...
                f"{st.session_state['audit_data']['schema_count']} items"
            ]
        })
    ...
    with col1:
        st.download_button("📥 Download Report", data=buffer, file_name="Audit_Report.xlsx")
    with col2:
